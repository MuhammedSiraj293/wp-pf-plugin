<?php
/**
 * TCA Unit Sales Offer – iPhone / iPad print page
 * Dynamically loads user offer inputs from local storage and prints.
 */
...
$expected = hash_hmac('sha256', 'offer_' . $unit_id, wp_salt('auth'));
...
$gallery_b64 = array_map('tca_oprint_img', $gallery_images);
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pdf-spec-item {
    flex: 1; background: #fff; border: 1px solid #e1e8ed; border-radius: 6px;
    padding: 5px 4px; text-align: center; box-sizing: border-box;
  }
  ...
  .pdf-amenity-checklist {
    display: flex; flex-wrap: wrap; justify-content: space-between; gap: 5px 0;
  }
  .pdf-amenity-check {
    width: 48%; box-sizing: border-box; font-size: 12px; color: #0a3c61;
  ...
  .pdf-details-grid { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 10px 0; }
  .pdf-detail-row {
    width: 48%; display: flex; justify-content: space-between; box-sizing: border-box;
...
<div class="page">
  ...
  <div class="content">
    ...
    <!-- Description -->
    <?php if ($description): ?>
    <div class="pdf-section" id="pdf-overview-section">
    ...
    <div class="pdf-section" id="pdf-amenities-section">
    ...
    <!-- Property Reference -->
    <div class="pdf-section" id="pdf-reference-section">
...
document.addEventListener('DOMContentLoaded', function() {
    const layoutBase64 = localStorage.getItem('tca_offer_layout');
    const paymentHeadersJSON = localStorage.getItem('tca_offer_payment_headers');
    const paymentPlanJSON = localStorage.getItem('tca_offer_payment_plan');
    
    const contentDiv = document.querySelector('.content');
    const footerDiv = document.querySelector('.pdf-footer');
    
    if (layoutBase64 || paymentPlanJSON) {
        // Remaining Payment
        if (paymentPlanJSON) {
            try {
                const headers = JSON.parse(paymentHeadersJSON || '["Milestone", "Percentage", "Period"]');
                const plan = JSON.parse(paymentPlanJSON);
                if (plan && plan.length > 0) {
                    const offerDiv = document.createElement('div');
                    let tblHTML = `
                        <div class="pdf-section">
                            <div class="pdf-section-title" style="text-transform:none!important;">Remaining Payment</div>
                            <table class="payment-table" style="width:100%; border-collapse:collapse; font-size:13px;">
                                <thead>
                                    <tr style="background:#0a3c61; color:#fff;">
                    `;
                    headers.forEach((h, index) => {
                        let style = 'text-align:left; padding:8px 12px;';
                        if (index === 0) style += 'border-radius:4px 0 0 4px;';
                        else if (index === headers.length - 1) style += 'border-radius:0 4px 4px 0; text-align:right;';
                        else style += 'text-align:center;';
                        tblHTML += `<th style="${style}">${h}</th>`;
                    });
                    tblHTML += `
                                    </tr>
                                </thead>
                    ...
                    plan.forEach((row, rowIndex) => {
                        ...
                        tblHTML += `<tr style="background:${bg}; border-bottom:1px solid #e5e7eb;">`;
                        row.forEach((cell, cellIndex) => {
                            let style = 'padding:8px 12px; color:#111827;';
                            ...
                            tblHTML += `<td style="${style}">${cell}</td>`;
                        });
                        tblHTML += `</tr>`;
                    });
                    tblHTML += `</tbody></table></div>`;
                    offerDiv.innerHTML += tblHTML;
                    
                    const amenitiesSec = document.getElementById('pdf-amenities-section');
                    if (amenitiesSec) {
                        amenitiesSec.parentNode.insertBefore(offerDiv, amenitiesSec.nextSibling);
                    } else {
                        contentDiv.insertBefore(offerDiv, footerDiv);
                    }
                }
            } catch(e) {
                console.error(e);
        ...
        if (layoutBase64) {
            ...
            layoutDiv.innerHTML += `
                <div class="pdf-section" style="page-break-inside:avoid; text-align:center;">
                    <div class="pdf-section-title" style="text-transform:none!important; text-align:left;">Unit Layout</div>
                    <div style="text-align:center; margin-top:10px;">
                        <img src="${layoutBase64}" style="max-width:100%; max-height:450px; object-fit:contain; border:1px solid #e5e7eb; border-radius:6px; padding:10px;">
                    </div>
                </div>
            `;
            const overviewSec = document.getElementById('pdf-overview-section');
            if (overviewSec) {
                overviewSec.parentNode.insertBefore(layoutDiv, overviewSec.nextSibling);
            } else {
                contentDiv.insertBefore(layoutDiv, footerDiv);
            }
        }
    }
    
    // Clear Local Storage
    localStorage.removeItem('tca_offer_layout');
    localStorage.removeItem('tca_offer_payment_headers');
    localStorage.removeItem('tca_offer_payment_plan');
