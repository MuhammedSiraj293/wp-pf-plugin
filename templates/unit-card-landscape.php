<?php
/**
 * Template for displaying a Landscape style property card.
 * Designed to explicitly match user's Image 1 screenshot.
 */
...
if (!$agent_avatar) $agent_avatar = "https://ui-avatars.com/api/?name=" . urlencode($agent_name ? $agent_name : 'Agent') . "&background=1e293b&color=fff";
...
$purpose_name = ( $purposes && ! is_wp_error( $purposes ) ) ? $purposes[0]->name : ''; // Can be Buy or Rent
...
?>
<div class="tca-card-landscape">
    <div class="tca-cl-image-box">
        ...
        <div class="tca-cg-carousel-controls">
                <span class="ctrl-left"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg></span>
                <span class="ctrl-right"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg></span>
            </div>
    ...
    <div class="tca-cl-content" style="padding: 20px; position: relative; display: flex; flex-direction: column; flex-grow: 1;">
        <div class="tca-cl-header-row" style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px;">
            <div class="tca-cl-title-price" style="min-width: 0; flex: 1;">
                <div class="tca-cl-listed" style="font-size:14px; color:#666; font-weight:500; margin-bottom:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                    <?php echo esc_html($project_name); ?>
                </div>
                <h2 class="tca-cl-price" style="margin: 5px 0 10px 0; font-size: 30px; font-weight: 500; line-height: 1; color: var(--tca-secondary);">
                    <?php echo number_format_i18n((float)$price); ?> AED<?php echo (strtolower($purpose_name)=='rent') ? '/Year' : ''; ?>
                </h2>
            </div>
            <?php if (!empty($developer_logo_url)): ?>
                <div class="tca-cl-developer-logo-flex-item" title="<?php echo esc_attr($developer_name); ?>" style="flex-shrink:0; margin-left:12px; background: #fff; padding: 4px; border-radius: 4px; border: 1px solid #e1e8ed; display: flex; align-items: center; justify-content: center;">
                    <img src="<?php echo esc_url($developer_logo_url); ?>" alt="<?php echo esc_attr($developer_name); ?>" style="max-height: 40px; max-width: 70px; object-fit: contain; display: block;">
                </div>
            <?php endif; ?>
        </div>

        <div class="tca-cl-title">
        ...
        <div class="tca-cg-specs" style="display:flex; flex-wrap:wrap; gap:12px; align-items:center; font-size:13px; color:#555; margin-bottom:12px; border-bottom:1px solid #f0f0f0; padding-bottom:12px;">
            ...
            <?php if ($price_sqft): ?>
                <span class="tca-cl-price-sqft" style="display:flex; align-items:center; gap:4px;">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                        <rect x="4" y="4" width="16" height="16" rx="2" ry="2"/>
                        <circle cx="12" cy="12" r="3"/>
                    </svg>
                    <?php echo number_format_i18n((float) $price_sqft); ?> AED/sqft
                </span> <span class="tca-sep tca-cl-price-sqft-sep">|</span>
            <?php endif; ?>
        ...
        <div class="tca-cl-footer">
            <div class="tca-cl-agent-bar">
            </div>
 
            <div class="tca-cg-actions">
                <?php 
                ...
                <a href="<?php echo $wa_link; ?>" target="_blank" class="tca-cg-btn tca-cg-btn-wa" style="flex:1;">
